...
// 実質main----------------------------------------------
//ランダムでレシピを取り出す
func Choice_RandomMenue(choice_num int, arg *FuncInitArg)  {
  ...
  if err != nil {
	...
	// 一週間分のメニューを作成
	Create_week_menu(random_ID_list, &GreetOpts)
}
// --------------------------------------------------------------
...
func Create_week_menu(recipe_id_list []int64, arg *FuncInitArg){
	is_init := false
	if arg.IsInit != nil {
		is_init = arg.IsInit
	}
